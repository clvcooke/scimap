...
function validateDate(dateStr) {
  ...
  if (!isoDateRegex.test(dateStr)) {
    return {
      valid: false,
      error: `Invalid date format: "${dateStr}". Expected YYYY-MM-DD format.`,
    }
  }
  ...
  if (isNaN(date.getTime())) {
    return {
      valid: false,
      error: `Invalid date: "${dateStr}". Expected YYYY-MM-DD format.`,
    }
  }
  ...
  if (date.getFullYear() !== year || date.getMonth() + 1 !== month || date.getDate() !== day) {
    return {
      valid: false,
      error: `Invalid date: "${dateStr}". Day/month combination is not valid.`,
    }
  }
...
function processSheetData(rows) {
  ...
  rows.forEach((row, index) => {
    ...
    if (!dateStr || !link || !title) {
      errors.push(
        `Row ${rowNumber}: Missing required field(s). Date="${dateStr || 'MISSING'}", Link="${link || 'MISSING'}", Title="${title || 'MISSING'}"`,
      )
      return
    }
...
main().then(() => console.log('Script completed successfully'))
